...
dfall <- read.csv("outputdata/df_all_predicted.csv", sep=",", stringsAsFactors=FALSE)
dfall$maxPosition <- apply(dfall[,c("Ala","AlaArmador","AlaPivo","Armador","Pivo")], 1, max) 
...
table(dfall$type, dfall$Temporada)

write.csv(dfall, "outputdata/classified_players.csv", fileEncoding = "UTF-8")
