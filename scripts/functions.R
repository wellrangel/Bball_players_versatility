...
readData<-function()
{
  round(0.2037234043*100,2)
  round(0.2142483953*100,2)
  
  #read csv
  totaldados <- read.csv("outputdata/aggregateData.csv", sep = "," ,stringsAsFactors=FALSE)
  
  
  totaldados[is.na(totaldados)] <- 0
  totaldados <- totaldados[totaldados$id_jogador > 0,]
  totaldados <- totaldados[totaldados$pos != "NOTFOUND",]
  totaldados <- totaldados[totaldados$pos != "0",]
  totaldados <- totaldados[totaldados$pos != "NOTFOUNDCONFIRMED",]
  totaldados <- totaldados[totaldados$pos != "",]

  totaldados <-totaldados[totaldados$played_minutes > 100,]
  
  
  totaldados[,c("pos")] <- as.factor(totaldados[,c("pos")])
...
GetDetalheMapa<-function (DetalheMapa, intervalo, posx, valor, total )
{
  ...
  if (valor != "0"){
    ...
    perc = round(perc,2)
    perctemp = round(perc,2)
    perc = as.character(perc)
    if (nchar(perc) == 4 && perctemp >10 ){
      perc = paste0(perc, "0")
    }
      
    resultado = paste(perc, "% (",  valor, ")", sep="")  
