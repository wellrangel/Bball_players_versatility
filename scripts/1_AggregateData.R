library(sqldf)
#set locale to Brazil Portuguese 
Sys.setlocale("LC_ALL","pt_BR.UTF-8")
#reading csv file with 9 seasons in nbb
dfstats <- read.csv("inputdata/all-stats-nbb.csv", sep = "," ,stringsAsFactors=FALSE)
...
NROW(unique(dfstats$id_jogador)) # 641
NROW(dfstats)  #42,110

#keeping only players over 5 minutes played per game
dfstats <-dfstats[dfstats$played_minutes >= 5,]#42110

NROW(unique(dfstats$id_jogador)) # 641
NROW(dfstats) #42110

#creating three new columns: made_two_points, made_three_points and made_free_throw_points
...
dfstats_by_years <- sqldf("SELECT distinct(jogador), id_jogador, jogador, jogador_apelido, Temporada, id_equipe, equipe,  
  sum(played_minutes) as played_minutes, 
...
sum(tried_points) as tried_points, sum(lost_points) as lost_points, sum(total_points) as total_points,
...
dfstats_by_yearsPosTemp <- read.csv("inputdata/playersPosition.csv", sep = ";" ,stringsAsFactors=FALSE, encoding = "UTF-8")
...
#save file
write.csv(dfstats_by_years, file = "outputdata/aggregateData.csv", fileEncoding = "UTF-8")
